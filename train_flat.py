...
# from models.lstm_ica import LSTM_ICA
...
fileStr = "D:\\user\\xxh\\研究or项目\\LSTM\\LSTM汇总\\代码\\实验matlab\\flat\\flat_with_sequence\\data\\flat_selected.csv"  # 换训练数据时注意改文件名
modelStr = "base"
num = 17
logStr = "flat_logs\\" + modelStr + "_" + str(num) + "_log.txt"
myEpoch = 4000
input_dim = 17+2
batch_size = 128
...
def main(modelStr, num):
    ...
    # 复制第一列和最后一列
    first_column = selected_data[:, 0:1]  # 第一列
    last_column = selected_data[:, -1:]  # 最后一列
    # 拼接数据：将第一列添加到前面，最后一列添加到后面
    ...
    if modelStr == "transformer":
        ...
        model = lstmModel(
            input_dim=input_dim, hidden_dim=64, lstm_dim=64, output_dim=input_dim
        )
        ...
        model_eval = GRuModel(input_dim=input_dim)
    # elif modelStr=="lstm_ica":
    #     model=LSTM_ICA(input_dim=input_dim,lstm_hidden_dim=64,ica_dim=input_dim,output_dim=input_dim)
    #     model_eval=LSTM_ICA(input_dim=input_dim,lstm_hidden_dim=64,ica_dim=input_dim,output_dim=input_dim)
    ...
    for epoch in range(num_epochs):
        ...
        if avg_loss < min_loss:
            ...
            torch.save(
                model.state_dict(),
                "flat_outputs/" + modelStr + "_best_model" + str(num) + ".pth",
            )
            model = model.to(device)
    ...
    # 为每个自由度计算加速度
    response = []
    for i in range(input_dim):  # 遍历所有列
        print(i)
        aa = data.iloc[pre:, i]
