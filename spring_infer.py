...
# ======== 配置区（只需修改这里） ========
DATA_FILE  = "D:\\user\\xxh\\研究or项目\\LSTM\\LSTM汇总\\代码\\实验matlab\\spring truss\\spring.csv"
MODEL_PTH  = "spring_outputs/lstm6.pth"
INPUT_DIM  = 6               # 与训练时保持一致
PLOT       = False           # 是否绘制各分量图
TIMING_LOG = "spring_outputs/timing_log.csv"   # 所有模型计时结果汇总
# ========================================
...
def build_model(model_type: str, input_dim: int) -> torch.nn.Module:
    if model_type == "base":
        ...
        return lstmModel(input_dim=input_dim, hidden_dim=64, lstm_dim=64, output_dim=input_dim)
    elif model_type == "gru":
        return GRuModel(input_dim=input_dim, hidden_dim=64, gru_dim=64, output_dim=input_dim)
    elif model_type == "transformer":
        # nhead 用推理时的 model_eval 配置（nhead=6）
        return TransformerModel(input_dim=input_dim, dim_feedforward=1024,
                                max_seq_length=15000, d_model=36,
                                nhead=6, num_encoder_layers=6)
    else:
