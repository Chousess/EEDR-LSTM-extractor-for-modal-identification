...
fileStr = r"D:\\user\\xxh\\研究or项目\\LSTM\\LSTM汇总\\代码\\实验matlab\\spring truss\\spring_AIAA.csv"  # 换训练数据时注意改文件名
modelStr = "lstm"
num = 6
...
def main(modelStr, num):
    ...
    if modelStr == "base":
        ...
        model = TransformerModel(dim_feedforward=1024,max_seq_length=15000,d_model=36,input_dim=input_dim,nhead=input_dim,num_encoder_layers=6)
        model_eval=TransformerModel(dim_feedforward=1024,max_seq_length=15000,d_model=36,input_dim=input_dim,nhead=6,num_encoder_layers=6)
    ...
    for epoch in range(num_epochs):
        ...
        if avg_loss < min_loss:
            ...
            torch.save(
                model.state_dict(),
                "spring_outputs/" + modelStr + str(num) + ".pth",
            )
            model = model.to(device)
