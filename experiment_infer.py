...
# ======== 配置区（只需修改这里） ========
DATA_FILE  = "D:\\user\\xxh\\研究or项目\\LSTM\\LSTM汇总\\代码\\实验matlab\\experiment\\experiment_5.csv"
MODEL_PTH  = "experiment_outputs/transformer4.pth"
INPUT_DIM  = 4               # 与训练时保持一致
PLOT       = False           # 是否绘制各分量图
TIMING_LOG = "experiment_outputs/timing_log.csv"   # 所有模型计时结果汇总
# ========================================

# MODEL_TYPE 和 OUTPUT_CSV 根据 MODEL_PTH 自动推断，无需手动填写
...
def build_model(model_type: str, input_dim: int) -> torch.nn.Module:
    if model_type == "base":
        ...
        # experiment 训练时用 hidden_dim=32, lstm_dim=32
        return lstmModel(input_dim=input_dim, hidden_dim=32, lstm_dim=32, output_dim=input_dim)
    elif model_type == "gru":
        # experiment 训练时显式传入所有参数
        return GRuModel(input_dim=input_dim, hidden_dim=16, gru_dim=16, output_dim=input_dim)
    elif model_type == "transformer":
        return TransformerModel(input_dim=input_dim, dim_feedforward=1024, max_seq_length=15000, d_model=36, num_encoder_layers=6)
    else:
...
def preprocess(data_file: str):
    """与训练时完全相同的预处理：直接 StandardScaler，不扩充首尾列。"""
