...
# ======== 配置区（只需修改这里） ========
DATA_FILE  = "D:\\user\\xxh\\研究or项目\\LSTM\\LSTM汇总\\代码\\实验matlab\\flat\\flat_with_sequence\\data\\flat_selected.csv"
MODEL_PTH  = "flat_outputs/lstm_best_model12.pth"
INPUT_DIM  = 17+2        # 与训练时保持一致
PLOT       = False       # 是否绘制各分量图
TIMING_LOG = "flat_outputs/timing_log.csv"   # 所有模型计时结果汇总
# ========================================
...
def _auto_model_type(pth_path: str) -> str:
    ...
    for t in _KNOWN_TYPES:          # 长名优先，避免 lstm_rsnet 被识别成 lstm
...
def _auto_output_csv(pth_path: str, model_type: str) -> str:
    ...
    match = re.search(r'\d+$', stem)          # 取末尾编号，如 17、12、4
...
def preprocess(data_file: str):
    """与训练时完全相同的预处理：扩充首尾列 + StandardScaler。"""
    df = pd.read_csv(data_file)
    raw = df.values
    data_extended = np.hstack([raw[:, 0:1], raw, raw[:, -1:]])
